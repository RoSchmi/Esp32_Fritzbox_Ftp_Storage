// Program 'Esp32_Fritzbox_Ftp_Storage' Branch Master
#define PROGRAMVERSION "v1.1.0"
// Last updated: 2024_08_07
...
// Default Esp32 stack size of 8192 byte is not enough if https was used in this App.
...
// Only needed if a microphone is needed as a sensor
...
#define AZURE_CONFIG_ACCOUNT_KEY   "YourStorageAccountKey"                                               // 'CONFIG_FILENAME' is used for Router Credentials
...
void loop() 
{
  ...
  if (++loopCounter % 100000 == 0)   // Make decisions to send data every 100000 th round and toggle Led to signal that App is running
  {
    ...
    digitalWrite(LED_BUILTIN, ledState);    // toggle LED to signal that App is running
      ...
      if (dataContainer.hasToBeSent() || isLast15SecondsOfDay)
      {
        insertCounterAnalogTable++;
        SampleValueSet sampleValue = dataContainer.getSampleValues(dateTimeUTCNow);        
        Serial.printf("InLoop: %.0f %f  %i-%02d-%02d %02d:%02d \n",sampleValue.SampleValues[2].Value * 10, sampleValue.SampleValues[0].Value, localTime.year(), localTime.month(), localTime.day(), localTime.hour(), localTime.minute());
      }      
  } 
}
